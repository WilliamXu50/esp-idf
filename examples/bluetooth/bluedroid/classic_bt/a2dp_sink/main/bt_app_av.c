...
{
    //This code modifies the PCM audio samples with accordance to the volume
    //The modified PCM audio stream is then sent to the ring buffer
    if(len!=0){
        int16_t* pcm_data=(int16_t*)data;
        for(int i=0; i<len/2;i++){
            int32_t temp=(int32_t)(*pcm_data);
            temp = temp * s_volume;
			temp = temp/512;
            *pcm_data=(int16_t)temp;
            pcm_data++;
        }
...
{
    //printf("Test message: AVRCP.attr_text: 0x%x\n", param->meta_rsp.attr_text);
...
static void bt_av_hdl_avrc_ct_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_AVRC_CT_METADATA_RSP_EVT: {
        ...
        if(rc->meta_rsp.attr_id==0x01){
            memset(lcd_dsp_line3, 0x20, LCD_DSP_LINE_MAX_CHAR);
            memset(lcd_dsp_line4, 0x20, LCD_DSP_LINE_MAX_CHAR);

            lcd_spi_send_cmd(lcd_spi_handle, 0x54|0x80);
            lcd_spi_send_string(lcd_spi_handle, lcd_dsp_line4, LCD_DSP_LINE_MAX_CHAR);
            ...
            if(rc->meta_rsp.attr_length){       //Data must be valid data, not garbage data
                ...
                //If song name exceeds 13 characters, only send the first 13 characters to the LCD
                ...
                //If song name is less than 13 characters, send all the characters of the song name  
                //and fill the empty slots with blank spaces (hex: 0x20)
                else{
        ...
        ESP_LOGI(BT_RC_CT_TAG, "AVRC metadata rsp: attribute length: %d", rc->meta_rsp.attr_length);
...
{
    ESP_LOGI(BT_RC_TG_TAG, "S_Volume = %d\n", s_volume);
...
/* No need for volume simulation since we got the actual volume control working */
